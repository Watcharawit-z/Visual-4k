...
## ทางลัด: ใช้ Visual4k-Setup.exe

ถ้าโหลด zip จากหน้า releases มา ไม่ต้องอ่านทั้งเอกสารนี้ **ดับเบิลคลิก
`Visual4k-Setup.exe`** แล้วเลือกข้อ 1 มันทำให้ครบทุกอย่าง:

- ขอสิทธิ์ Administrator เอง (กล่อง UAC จะเด้งขึ้นมา กด Yes)
- เอาใบรับรองของ build เข้า trust store
- เปิด test signing ให้ พร้อมอธิบายว่าแลกอะไรไป แล้วถามก่อนทำ และเสนอ reboot
- ติดตั้งไดรเวอร์ **และสร้างอุปกรณ์ให้** — ขั้นที่เดิมต้องใช้ devcon จาก WDK
  หรือเดินผ่าน Add legacy hardware ใน Device Manager หกขั้น
- ตั้งจอเสมือนเป็น 3840×2160 เปิดคอมโพสิเตอร์ **ก่อน** แล้วค่อยย้ายเดสก์ท็อป
  ไปจอเสมือน ลำดับนี้ตั้งใจให้พาเนลจริงไม่มีวินาทีไหนที่ว่างเปล่า
- นับถอยหลัง 20 วินาทีให้ยืนยัน ถ้าไม่กดจะคืนค่าจอกลับเอง

ข้อสุดท้ายสำคัญกว่าที่คิด: การย้อนอัตโนมัติ 15 วินาทีที่ Windows มีให้ เป็น
ความสามารถของแอป Settings ไม่ใช่ของ API ที่โปรแกรมเรียก การเปลี่ยนค่าจอจากโค้ด
มีผลถาวรทันที ตัวนับถอยหลังกับการย้อนกลับจึงต้องเป็นของเราเอง

เลือกข้อ 3 เมื่อไหร่ก็ถอนออกได้หมด รวมถึงปิด test signing คืนให้ด้วย

เอกสารที่เหลือคือทางยาว — สำหรับคนที่อยาก build เองหรืออยากรู้ว่าแต่ละขั้นทำอะไร
...
6. รันคอมโพสิเตอร์:
   ...
   ค่าเริ่มต้นคือ **อ่านจากจอหลัก** (ซึ่งตอนนี้คือจอเสมือน) แล้ว **วาดลงจออื่น**
   ถ้ามีจอต่ออยู่เกินสองตัว ค่าเริ่มต้นอาจเลือกจอผิด ระบุเองได้:

   ```powershell
   .\visual4k-host.exe --source \\.\DISPLAYn --output \\.\DISPLAYm
   ```

   `--source` คือจอเสมือน 4K, `--output` คือพาเนลจริงที่อยากให้ภาพไปโผล่
   ดูชื่อและตำแหน่งของแต่ละจอได้จาก `--list-displays`
